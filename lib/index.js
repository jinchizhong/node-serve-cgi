...
var default_options = {
  ...
  decorator: function(headers, body, done) {
    done(headers, body);
  },
...
module.exports = function(opts) {
  ...
  obj.respone = function(res, code, stdout, stderr, env) {
    if (code === 0) {
      ...
      opts.decorator(x[0], x[1], obj.send.bind(this, res));
    } else {
      var headers = {
      ...
      opts.decorator(headers, body, obj.send.bind(this, res));
    }
  };
  obj.handle = function(req, res, next) {
    if (req.url.replace(/\?.*/, '') === '/' && !req.originalUrl.replace(/\?.*/, '').match(/\/$/)) {
      res.redirect(req.originalUrl.replace(/\?.*/, '') + '/');
      return;
    }
    obj.match(url.parse(req.url).pathname, function(matched) {
      if (matched) {
        if (matched.isDirectory) {
          res.redirect(req.originalUrl.replace(/\?.*/, '') + '/');
        } else {
          var env = obj.req2env(req, matched.fpath, opts.root);
          obj.run(matched.cmd, opts.root, env, function(code, stdout, stderr) {
